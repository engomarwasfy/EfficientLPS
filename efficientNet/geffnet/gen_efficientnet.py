...
__all__ = ['GenEfficientNet', 'mnasnet_050', 'mnasnet_075', 'mnasnet_100', 'mnasnet_b1', 'mnasnet_140',
           ...
           'tf_efficientnet_b0', 'tf_efficientnet_b1', 'tf_efficientnet_b2', 'tf_efficientnet_b3', 'tf_efficientnet_ren',
...
class GenEfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, in_chans=3, num_features=1280, stem_size=32, fix_stem=False,
                 ...
                 se_kwargs=None, norm_layer=nn.BatchNorm2d, norm_kwargs=None, proximity=False,
                 weight_init='goog'):
        ...
        if self.proximity:
            ...
            self.range_weight = Parameter(torch.Tensor(
                    in_chans, stem_size, *(kernel_knn_size, kernel_knn_size) ))
            init.kaiming_uniform_(self.range_weight, a=math.sqrt(5))
            self.out_channels = stem_size
            self.knn_nr = kernel_knn_size ** 2
            self.conv_stem = select_conv2d(stem_size, stem_size, 3, stride=2, padding=pad_type)
        else:
...
#        self.classifier = nn.Linear(num_features, num_classes)
    ...
    def proximity_conv(self, inputs, mean, std):
        ...
        _, knn_idx = difference.topk(
                     self.knn_nr, dim=1, largest=False)
        ...
        return torch.matmul(
            self.range_weight.view(self.out_channels, -1), unfold_inputs
        ).view(B, self.out_channels, H, W)

    def features(self, x, mean=None, std=None):
        endpoints = []

        if self.proximity:
            x = self.proximity_conv(x, mean, std)
        x = self.conv_stem(x)
        x = self.bn1(x)
        x = self.act1(x)
...
def tf_efficientnet_ren(pretrained=False, **kwargs):
