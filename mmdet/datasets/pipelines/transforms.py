...
class Resize(object):
    ...
    def __init__(self,
        ...
        self.upsample = torch.nn.Upsample(size=img_scale[::-1],mode='bilinear')    
    ...
    def _resize_img(self, results):
        if self.keep_ratio:
            ...
            if results['img'].shape[0] == 5:
                 img = self.upsample(results['img'].unsqueeze(0)).squeeze(0)
                 w_scale = img.shape[2]/float(results['img'].shape[2])
                 h_scale = img.shape[1]/float(results['img'].shape[1])
                 img_shape = (img.shape[1], img.shape[2], img.shape[0])    
            else:
        ...
        results['pad_shape'] = img_shape  # in case that there is no padding
