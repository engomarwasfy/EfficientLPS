...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        if 'img' in results and results['img'].shape[0] < 5:
            img = results['img']
            if len(img.shape) < 3:
                img = np.expand_dims(img, -1)
...
class Collect(object):
    ...
    def __init__(self,
                 ...
                 meta_keys=('filename', 'ori_shape', 'img_shape', 'pad_shape',
                            'scale_factor', 'sensor_img_means', 'sensor_img_stds', 
                            'proj_msk', 'stuff_id', 'proj_x', 'proj_y', 'proj_range',
                            'unproj_range', 'unproj_n_points')):
        self.keys = keys
